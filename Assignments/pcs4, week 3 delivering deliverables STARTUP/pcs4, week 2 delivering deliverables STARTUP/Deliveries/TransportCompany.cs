...
namespace Deliveries
{
    class TransportCompany
    {
        ...
        private List<Deliverable> sortedlist, sortedlist1;
        ...
        public Deliverable BinarySearchById(int id, int lowindex, int highindex)
        {
                ...
                if (sortedlist[m].ID == id)
                {
                    
                    return (Deliverable)sortedlist[m];
                    
                }

                else if (sortedlist[m].ID > id)
                {
                   return BinarySearchById(id, lowindex, m - 1);
                }

                 else if (sortedlist[m].ID < id)
                {
                    return BinarySearchById(id, m + 1, highindex);
                }
        ...
        public Deliverable BinarySearchByName(string name, int lowindex, int highindex)
        {while (lowindex <= highindex)
            {
            int m = (lowindex + highindex) / 2;
            
                if (sortedlist[m].Buyer.Name == name)
                {
                ...
                else if (string.Compare(sortedlist[m].Buyer.Name, name) > 0)
                {
        ...
        public List<Deliverable> FindAllDeliverables(String buyersName)
        {
            ...
            SortByName();
            Deliverable d = BinarySearchByName(buyersName, 0, SortedList.Count - 1);
            if (d != null)
            {
                temp.Add(d);
                sortedlist.Remove(d);
                
                d = BinarySearchByName(buyersName, 0, SortedList.Count - 1);
                while (d != null)
                {
                    temp.Add(d);
                    sortedlist.Remove(d);
        ...
        {
            sortedlist = Deliverables;
            nameSort flupke = new nameSort();
            this.sortedlist.Sort(flupke);
        ...
        public void SortByAddress()
        {
           ...
           sortedlist.Sort(new Comparison<Deliverable>(this.CompareDrumblesByStreet));
           

        }

        public int CompareDrumblesByStreet(Deliverable firstDrumble, Deliverable secondDrumble)
        {
            if (string.Compare((firstDrumble.Buyer.Street),
                    (secondDrumble.Buyer.Street)) == 0)
            { return firstDrumble.Buyer.Housenumber.CompareTo(secondDrumble.Buyer.Housenumber); }
            else { return firstDrumble.Buyer.Street.CompareTo(secondDrumble.Buyer.Street); }
        }
        /// <summary>
